...
?>
<input type="text" hidden value="<?php echo Helper::routes('order.route.php'); ?>" id="url_order-f">
<input type="text" hidden value="<?php echo $userCurrent['id'] ?>" id="id_user-f">


<ul id="list-order-finished" class="space-y-4">

</ul>
<script>
    const id_user$ = document.getElementById('id_user-f').value;
    const url_order$ = document.getElementById('url_order-f').value;
    $(() => {
        console.log(id_user$,url_order$);
        $.get(url_order$, {
            method: "findByIdUserFinished",
            id_user: id_user$
        }, (data) => {
            try {
                console.log(data);
                ...
                console.log(orders);
                $('#list-order-finished ').html(orders?.map((order) => {
                    return `
